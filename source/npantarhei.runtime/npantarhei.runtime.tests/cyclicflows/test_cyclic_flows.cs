...
namespace npantarhei.runtime.tests.cyclicflows
{
	...
	public class test_cyclic_flows
	{
		...
		[Test()]
		public void test_cyclic_flow_with_EBC_fails ()
		{
		...
		public void test_cyclic_flow_with_async_EBC ()
		{
            ...
            var config = new FlowRuntimeConfiguration()
                                .AddStreamsFrom(@"
                                                    ...
                                                    cyclic.stopOn, .out
                                                    ")
                                .AddEventBasedComponent("cyclic", new CyclicFlow()).MakeAsync();
            ...
            using(var fr = new FlowRuntime(config))
            {
				...
				// async EBC does not work comparing to native actions (see test_cyclic_flow_with_async_action), under clarfication...
                Assert.IsFalse(fr.WaitForResult(5000));
				
				// this is whar we expected to work:
//                IMessage result = null;
//                Assert.IsTrue(fr.WaitForResult(10000, _ => result = _));
